...
const { iconsReady } = await import('../src/kitIcons.ts')
const { appAssetsReady } = await import('../src/apps/assets.ts')
const { paintApp } = await import('../src/apps/render.ts')
...
const html = `<!doctype html><html lang="en"><meta charset="utf-8"><title>Windows 95 pixel comparison</title><style>body{font:14px system-ui;margin:32px;color:#111;background:white}article{margin:36px 0}section{display:flex;gap:20px;flex-wrap:wrap}figure{margin:0}img{image-rendering:pixelated;max-width:100%}figcaption{margin:8px 0}p{max-width:850px;line-height:1.5}</style><h1>Windows 95 desktop — pixel comparison</h1><p>Source screenshots: <a href="https://guidebookgallery.org/screenshots/win95/">GUIdebook Windows 95</a> (Calculator and Control Panel) and <a href="https://guidebookgallery.org/screenshots/win95osr2/">Windows 95 OSR2</a> (Internet Explorer 3). The app renderer runs in @napi-rs/canvas for reproducible, lossless output; live browser interactions are checked separately. Exact RGBA comparison at each source's original dimensions. No tolerance, alignment adjustment, masking, or excluded pixels. Magenta marks every mismatch. Icons are extracted original artwork; chrome, text and layout are independently painted by the same renderer used on the desktop.</p><p>These are the initial active windows at their native sizes. Interactive states and settings dialogs are not covered by these source screenshots. Large flat areas and reused artwork contribute to the score; the percentages alone do not establish perceptual fidelity.</p>${results.map(r => `<article><h2>${applications[r.id].title} — ${r.exactMatchPercent}% identical pixels</h2><p>${r.different.toLocaleString()} differing pixels / ${r.pixels.toLocaleString()}. ${r.width} × ${r.height}. Chrome: ${r.regions.chrome.different} differences. Content: ${r.regions.content.different} differences.</p><section><figure><img alt="Original screenshot" src="../apps/${r.id}.png" width="${r.width}" height="${r.height}"><figcaption>Source</figcaption></figure><figure><img alt="Recreated window" src="${r.id}-actual.png" width="${r.width}" height="${r.height}"><figcaption>Implementation</figcaption></figure><figure><img alt="Pixel differences" src="${r.id}-diff.png" width="${r.width}" height="${r.height}"><figcaption>Exact pixel difference</figcaption></figure></section></article>`).join('')}</html>`
