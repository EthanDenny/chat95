...
test('chat forwards history and server credentials, returning only reply text and model', async () => {
  ...
  await withServer(async (url, init) => {
    ...
    assert.match(body.messages[0].content, /Interpret present-tense questions as questions about today in 1996/)
    assert.match(body.messages[0].content, /Prime Minister of Canada is Jean Chrétien/)
    assert.doesNotMatch(body.messages[0].content, /Prime Minister of Canada was Jean Chrétien/)
...
test('system prompt uses the current month and day in 1996 while keeping the knowledge boundary separate', () => {
  ...
  assert.match(leapDay, /Treat the latest status you know as the current status/)
  assert.match(leapDay, /Never preface an ordinary answer with "as of 1994"/)
