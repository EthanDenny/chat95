...
These are distinct typefaces. System is not MS Sans Serif with a bold flag. Fixedsys is a monospaced family. The catalog contains the 218 printable Unicode-mapped Windows-1252 glyphs from each of the eight native strikes. Unsupported characters use a visible question-mark fallback; this is not a full-Unicode font collection.
...
Verification round-trips every shipped outline and metric against the source: 218 glyphs per face, 3,052 total. Exact glyph geometry is verified; browser/OS rasterization still requires visual validation. Characters outside the source repertoire can fall back to a browser font so native editing remains usable. The offline screenshot comparisons validate the reference bitmap renderer, not browser text rasterization.
