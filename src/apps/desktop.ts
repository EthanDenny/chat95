...
export function drawDesktop(ctx: CanvasRenderingContext2D, state: DesktopState, tracking: Rect | null = null) {
  ...
  state.windows.filter(w => !w.minimized).forEach(w => paintApp(ctx, w, state.apps, w.id === active, state.pressed?.app === w.id ? state.pressed.id : null, state.editingAddress))
