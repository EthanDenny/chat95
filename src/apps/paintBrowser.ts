...
import type { AppControl, BrowserState } from './model'
...
export function paintBrowser(ctx: CanvasRenderingContext2D, width: number, height: number, state: BrowserState, pressed: string | null, editing = false) {
  ...
  p.clip({ x: 62, y: 90, width: width - 133, height: 18 }, () => p.text(state.address, 63, 93))
  p.scrollArrow({ x: width - 71, y: 90, width: 16, height: 18 }, 'down')
  if (editing) p.clip({ x: 62, y: 90, width: width - 133, height: 18 }, () => {
