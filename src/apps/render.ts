import { panelControls } from './panelLayout'
import { createPainter, palette } from '../win95'
import { paintChrome, chromeControls, menuRects } from './chrome'
import { paintCalculator, calculatorButtons } from './paintCalculator'
...
import type { AppId, AppControl, DesktopApps, AppWindow } from './model'
...
export function paintApp(ctx: CanvasRenderingContext2D, w: AppWindow, apps: DesktopApps, active = true, pressed: string | null = null, editing = false) {
  ctx.save(); ctx.beginPath(); ctx.rect(w.x, w.y, w.width, w.height); ctx.clip(); ctx.translate(w.x, w.y)
  paintChrome(ctx, w.id, w.width, w.height, active, pressed, w.maximized)
  if (w.id === 'calculator') paintCalculator(ctx, apps.calculator, pressed)
  if (w.id === 'control-panel') paintPanel(ctx, w.width, w.height, apps.selection, apps.panelScroll)
  if (w.id === 'browser') paintBrowser(ctx, w.width, w.height, apps.browser, pressed, editing)
  ...
  if (notice || (w.id === 'control-panel' && apps.applet !== null)) {
    ...
    p.pushButton({ x: r.x + r.width - 87, y: r.y + r.height - 36, width: 75, height: 23 }, 'OK', pressed === 'dialog-close' ? 'pressed' : 'preferred')
  }
  if (apps.focused?.app === w.id) {
    const control = appControls(w, apps).find(c => c.id === apps.focused!.id)
    if (control) p.focus(control)
  }
  ctx.restore()
}
